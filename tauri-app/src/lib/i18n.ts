...
const ja = {
  ...
  templates: {
    ...
    nameLabel: "テンプレート名",
    namePlaceholder: "例: daily",
    nameRequired: "名前を入れると保存できます。名前は Note との紐付けに使われます",
    nameEmpty: "名前を入れてください",
    titleLabel: "Note のタイトル",
    titlePlaceholder: "タイトル",
    bodyLabel: "本文",
    rename: "名前を変更",
    renameNew: "新しい名前",
    renameWarn: (count: number) =>
      `このテンプレートから作った ${count} 件の Note との紐付けが切れます。Note 自体は残ります。`,
    renameNone: "このテンプレートから作った Note はまだありません。",
    renamed: (name: string) => `名前を「${name}」に変更しました`,
    renameFailed: "名前を変更できませんでした",
    more: "その他の操作",
    noteCount: (count: number) => `${count} 件`,
    ...
    varPrev: "前回の Note",
    varExample: "記入例",
    varDateHint: (example: string) => `例: ${example}（{{date:YYYY-MM}} で書式指定）`,
    varTimeHint: (example: string) => `例: ${example}`,
    varWeekdayHint: (example: string) => `例: ${example}`,
    varPrevHint: "前回の Note へのリンク。なければ行ごと消えます",
    varExampleHint: "次の空行か見出しまでが記入例。Note には書き出されません",
    varHint: "{{ と打つと変数の候補が出ます · 薄い行は記入例で、Note には書き出されません",
    insertInto: (field: string) => `${field}に挿入`,
    fieldTitle: "タイトル",
    fieldTag: "タグ",
    fieldBody: "本文",
    todayPreview: "今日作ると",
...
const en: Messages = {
  ...
  templates: {
    ...
    namePlaceholder: "e.g. daily",
    nameRequired: "Name it to save. The name is what links it to the Notes made from it",
    ...
    renameWarn: (count: number) =>
      count === 1
        ? "The 1 Note made from this template loses its link to it. The Note itself stays."
        : `The ${count} Notes made from this template lose their link to it. The Notes themselves stay.`,
    renameNone: "No Note has been made from this template yet.",
    renamed: (name: string) => `Renamed to “${name}”`,
    renameFailed: "Could not rename the template",
    more: "More actions",
    ...
    varExample: "example",
    varDateHint: (example: string) => `e.g. ${example} ({{date:YYYY-MM}} sets the format)`,
    varTimeHint: (example: string) => `e.g. ${example}`,
    varWeekdayHint: (example: string) => `e.g. ${example}`,
    varPrevHint: "A link to the previous Note. Without one, the whole line goes",
    varExampleHint:
      "Up to the next blank line or heading is an example, never written into the Note",
    varHint:
      "Type {{ for the variables · Faint lines are examples and are not written into the Note",
    insertInto: (field: string) => `Insert into ${field}`,
    fieldTitle: "title",
    fieldTag: "tag",
    fieldBody: "body",
