...
describe("outlineToTree", () => {
  it("空の本文は空のルートになる", () => {
    expect(outlineToTree("")).toStrictEqual({ content: "", children: [] });
  });
  ...
  it("見出しの下のリストが枝になる", () => {
    ...
    expect(root.content).toBe("計画");
    expect(contents(root.children)).toStrictEqual(["買い出し", "仕込み"]);
  });
  ...
  it("リストの入れ子は枝の入れ子になる", () => {
    ...
    expect(contents(item.children)).toStrictEqual(["野菜", "肉"]);
  });
  ...
  it("同じ見出しの下でリストと小見出しが共存する", () => {
    ...
    expect(root.content).toBe("計画");
    expect(contents(root.children)).toStrictEqual(["買い出し", "当日"]);
    expect(contents(root.children[1].children)).toStrictEqual(["集合"]);
  });

  it("見出しの階層が枝の階層になる", () => {
    const root = outlineToTree(["# A", "## B", "### C", "## D"].join("\n"));
    expect(root.content).toBe("A");
    expect(contents(root.children)).toStrictEqual(["B", "D"]);
    expect(contents(root.children[0].children)).toStrictEqual(["C"]);
  });

  it("トップレベルが複数あるときは空のルートでまとめる", () => {
    const root = outlineToTree(["# A", "# B"].join("\n"));
    expect(root.content).toBe("");
    expect(contents(root.children)).toStrictEqual(["A", "B"]);
  });

  it("階層を飛ばした見出し(H1 の次に H3)も直近の親につく", () => {
    const root = outlineToTree(["# A", "### C"].join("\n"));
    expect(contents(root.children)).toStrictEqual(["C"]);
  });

  it("リスト項目の段落以外(本文の地の文)は構造に含めない", () => {
    const root = outlineToTree(["# 計画", "地の文の段落。", "- 買い出し"].join("\n"));
    expect(contents(root.children)).toStrictEqual(["買い出し"]);
  });
